...
MLP *make_mlp(int nb_layers, int hamming, int *nb_neurons, char *trans){
  int l,i;
  int *nb_weights=0;
  MLP *mlp=0;
  ...
  if (!(in=make_layer(hamming,0,(char)0))){
    free(mlp->layer);
    free(mlp);
    free(nb_weights);
    return(0);
...
bool manage_net(MN *mn, AudioSet *trainset, AudioSet *testset, char tnt_flag, trainmode b_o){
  ...
  if (tnt_flag&TRAINFL){
    ...
    for (file=0;file<trainset->nb_files;file++){  /* jedes file in AudioSet wird durch das Netz getrieben */
      /*       printf("%d\n",file); */
      if (!(train=audiovv(mn,&au,trainset->files[file]))){    /* macht gesamte audio-Vorverarbeitung fuer aktuelles file */
  ...
  if (tnt_flag&TESTFL){
	...
	return(false);
      ...
      while (curr[n]->next)
	/* 	  for (i=0;i<mlp[n]->stats.test_epochs;i++) */
	curr[n]=curr[n]->next;
    }
    if (!(errelem=(ListElem *)calloc(1,sizeof(ListElem))))
      return(false);
    ...
    for (file=0;file<trainset->nb_files;file++){
      if (!(test=audiovv(mn,&au,testset->files[file])))
	return(false);
      for (n=0;n<OU_N;n++){
	prop(mlp[n],test);
